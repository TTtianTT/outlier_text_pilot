...
def verify_readout(run, limit=100):
    ...
    ids = sorted({i for groups in selections.values() if groups for inds in groups.values() for i in inds
                  if rows[i]["split"] in {"test", "ood"}}, key=lambda i: stable_seed(cfg["seed"], rows[i]["candidate_id"], "verify"))[:limit]
    ...
    result = {"status": status, "backend": meta["feature_signature"]["backend"], "fresh_text_forwards": len(ids),
    ...
    write_json(run / "readout_verification.json", result)
    return result
